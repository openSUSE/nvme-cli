...
static bool nvme_show_simple_ns(const char *name, void *arg)
{
...
static void nvme_show_simple_list(nvme_root_t r)
{
	...
	strset_iterate(&res.namespaces, nvme_show_simple_ns, &res);
...
static bool nvme_show_detailed_name(const char *name, void *arg)
{
	bool *first = arg;
...
static bool nvme_show_detailed_subsys(const char *name, void *arg)
{
	struct nvme_resources *res = arg;
	...
	first = true;
	strset_iterate(&ctrls, nvme_show_detailed_name, &first);
	strset_clear(&ctrls);
	printf("\n");
...
static bool nvme_show_detailed_ctrl(const char *name, void *arg)
{
	struct nvme_resources *res = arg;
	struct strset namespaces;
	nvme_ctrl_t c;
	...
	first = true;
	strset_iterate(&namespaces, nvme_show_detailed_name, &first);
...
static bool nvme_show_detailed_ns(const char *name, void *arg)
{
	struct nvme_resources *res = arg;
	struct htable_ns_iter it;
	struct strset ctrls;
...
static void nvme_show_detailed_list(nvme_root_t r)
{
	...
	printf("%-.16s %-.96s %-.16s\n", dash, dash, dash);
	strset_iterate(&res.subsystems, nvme_show_detailed_subsys, &res);
	printf("\n");

	printf("%-8s %-20s %-40s %-8s %-6s %-14s %-12s %-16s\n", "Device",
		"SN", "MN", "FR", "TxPort", "Asdress", "Subsystem", "Namespaces");
	printf("%-.8s %-.20s %-.40s %-.8s %-.6s %-.14s %-.12s %-.16s\n", dash,
		dash, dash, dash, dash, dash, dash, dash);
	strset_iterate(&res.ctrls, nvme_show_detailed_ctrl, &res);
	printf("\n");
	...
	printf("%-.12s %-.12s %-.10s %-.26s %-.16s %-.16s\n", dash, dash, dash,
		dash, dash, dash);
	strset_iterate(&res.namespaces, nvme_show_detailed_ns, &res);
